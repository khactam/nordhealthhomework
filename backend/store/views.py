...
from store.serializers import ProductSerializer, CartSerializer, CartItemSerializer, CreateCartItemSerializer
...
def cartItemsApi(request, id=0):
    if request.method == 'GET':
        cartItems = CartItem.objects.filter(Cart_id = id).select_related('Product')
        ...
        isCartExisted = Cart.objects.filter(pk=cart_item_data["Cart_id"]).exists()
        isProductExisted = Product.objects.filter(pk=cart_item_data["Product_id"]).exists()
        if isCartExisted and isProductExisted:
            new_cart_item = CartItem.objects.create(Cart_id=cart_item_data["Cart_id"], Product_id=cart_item_data["Product_id"])
            new_cart_item.save()
